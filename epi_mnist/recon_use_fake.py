...
#cos_sim comparison
cos_sim = tf.transpose(tf.matmul(fake_z,tf.transpose(tf.stop_gradient(embed)))*mem_inv_mag)
#cos_sim = tf.transpose(tf.matmul(fake_z,tf.transpose(embed))*mem_inv_mag)
...
for i in range(int(1e7)):
    cur_input,cur_output = mnist.train.next_batch(1)
    _,_,*cur_loss,cur_acc,cur_embed = sess.run([train_step,helper_train_step,class_loss,class_loss,recon_loss,acc,embed],feed_dict={x_:cur_input,y_:cur_output,mem_embed_:M['embed'],mem_y_:M['label']})
